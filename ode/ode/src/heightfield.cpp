...
//            return 1;
            ...
            // For us, totally under heightfield means no collision.
            // This could potentially be an issue for the torso collision.
            // However, since the heightfield of the torso check is dense
            // the torso has to go through a wall to be completely underneath
            // the heightfield. So this should never happen.
            return 0;
        }
        if (allFinite && minY - minO2Height > -dEpsilon && maxO2Height - maxY > -dEpsilon) {
          // Object is both above and below height field. It has to collide somewhere.
          // We can do this check because we're not interested in the contact location.
          // Only valid when all triangles are finite.
          return 1;
        }
    }
    ...
    // The single plane check is only valid if all triangles are valid.
    if (allFinite) {
    ...
    */
    ...
    for ( x_local = 0; x_local < maxX_local; x_local++)
    {
        ...
        for ( z_local = 0; z_local < maxZ_local; z_local++)
        {
            ...
            const bool isAfinite = std::isfinite(AHeight);
            const bool isBfinite = std::isfinite(BHeight);
            const bool isCfinite = std::isfinite(CHeight);
            const bool isDfinite = std::isfinite(DHeight);

            const bool isACollide = (AHeight > minO2Height) && isAfinite;
            const bool isBCollide = (BHeight > minO2Height) && isBfinite;
            const bool isCCollide = (CHeight > minO2Height) && isCfinite;
            const bool isDCollide = (DHeight > minO2Height) && isDfinite;
            ...
            if ((isACollide || isBCollide || isCCollide) && (isAfinite && isBfinite && isCfinite))
            {
                if ( geomNDepthGetter )
                {
                    if (!A->state) {
                        const dVector3 &triVertex = A->vertex;
                    ...
                    if (!B->state) {
                        const dVector3 &triVertex = B->vertex;
                        depth = geomNDepthGetter(o2,
                                                 triVertex[0], triVertex[1], triVertex[2] );
                        if (depth > dEpsilon)
                            return 1;
                    ...
                    if (!C->state) {
                        const dVector3 &triVertex = C->vertex;
                        depth = geomNDepthGetter(o2,
                                                 triVertex[0], triVertex[1], triVertex[2] );
                        if (depth > dEpsilon)
                            return 1;
            ...
            if ((isBCollide || isCCollide || isDCollide) && (isBfinite && isCfinite && isDfinite))
            {
                if ( geomNDepthGetter )
                {
                    if (!B->state) {
                    ...
                    if (!D->state) {
                        const dVector3 &triVertex = D->vertex;
                        depth = geomNDepthGetter( o2,
                            triVertex[0], triVertex[1], triVertex[2] );
                        if (depth > dEpsilon)
                            return 1;
...
//    dIASSERT (numTri != 0);
    // We can actually have no valid triangles, such that numTri == 0 is possible.
    ...
    // then collide against that list of triangles.
    {
        ...
        // sort planes
        if (numPlanes && isContactNumPointsLimited)
